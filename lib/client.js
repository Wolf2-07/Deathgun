...
const executeCommand = async (command, conn, msg) => {
   const handleCommand = async (Instance, args) => {
      ...
      try {
         ...
         await conn.sendMessage(msg.from, { text: `Command execution failed bro.\n\n${errorMessage}\n\nIf this persists, please contact the Deathsgun0001 dev.` }).catch(sendError => console.error("Error sending error message:", sendError));
      }
   };
